...
final class Base64ExtensionException extends ValidationException
{
    ...
    protected $defaultTemplates = [
        self::MODE_DEFAULT  => [self::STANDARD => '{{name}} extension ({{extension}}) should match mime type ({{mimeType}})'],
        self::MODE_NEGATIVE => [self::STANDARD => '{{name}} extension ({{extension}}) should not match mime type ({{mimeType}})'],
    ];
}//end class
